...
(function(window) {
    return {
              ...
              //  this.drawing = true;
                ...
                if(this.useFirstToRowName){
                    ...
                    for(var i = 1; i < this.wkdata.length; i++){
                        ...
                        // this.data.push(this.wkdata[i].slice(1)); // @@@処理変更

                        // 二次配列にする
                        this.data[i-1] = [];
                        this.markerColorData[i-1] = [];
                        for (var j = 1; j < this.wkdata[i].length; ++j) {
                            if (this.wkdata[i][j] instanceof Array) {
                                this.data[i-1][j-1]      = this.wkdata[i][j][0];
                                this.markerColorData[i-1][j-1] = this.wkdata[i][j][1];
                            }
                            else {
                                this.data[i-1][j-1]      = this.wkdata[i][j];
                                this.markerColorData[i-1][j-1] = null;
                            }
                        }
                    }
                }
            ...
            drawAxisX: function () {
                ...
                //水平目盛線を下辺から描いていく
                for (var count = 0, top = this.chartBottom - (this.yGap*count); this.chartTop <= top; ++count, top = this.chartBottom - (this.yGap*count)) {
                    if (this.adds.length <=2) {
                        this.ctx.beginPath();
            ...
            drawMarkers: function (op) {
                ...
                if(this.type==='scatter'){
                  for (var l = 0; l < this.dataColLen; l++) {
                    ...
                    var y = (this.chartBottom - (posy - this.minY * this.unitH));
                      //draw
                    _drawmarkers(x||0, y||0, 0, l, data, colorSet, colorIndex, this.markerColorData);
                  }
                } else {
                    for (var k = 0; k < this.dataRowLen; k++) {
                        var x = this.chartLeft;
                        ...
                        for (var l = 0; l < data[k].length; l++) {
                            ...
                            var y = this.chartBottom - (posy - this.minY * this.unitH);
                            //draw
                            _drawmarkers(x, y, k, l, data, colorSet, colorIndex, this.markerColorData);
                            x += this.xGap;
                        }
                    }
                }
                ...
                function _drawmarkers(x, y, row, col, data, colorSet, colorIndex, markerColorData){
                    that.ctx.beginPath();
                    that.ctx.fillStyle = colorSet[colorIndex];
                    // @@@ 配列にてカラーが指定されていたら色を変更する
                    if (markerColorData[row][col] !== null) {
                        that.ctx.fillStyle = markerColorData[row][col];
                    }
